...
{//getURLparameterset
	...
	{this.width = 980;
		this.height = 545;
		Q.addMeta({name:"viewport", content:"user-scalable=no"});}else
	...
	//load info
	var div = Q.createDOM("div", {innerHTML: "textmiddle, text...<br>", style:
	{id: "loader",
		position: "absolute",
		width: this.width + "px",
		left: "0px",
		...
		textAlign: "center",
		color: "#fff",
		font: Q.isMobile? 'bold 16px Arial': 'bold 16px Arial',
...
{var content = "textmiddle, text...<br>(" + Math.round(e.target.getLoadedSize()/e.target.getTotalSize()*100) + "%)";
...
{var me = this;
	...
	{var canvas = Q.createDOM("canvas", {id:"canvas", width:this.width, height:this.height, style:{position:"absolute"}});
		...
		this.context = new Q.CanvasContext({canvas:canvas});}else
