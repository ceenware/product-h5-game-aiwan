function initSplash() {gameState = "splash", allowSound &&!muted && music.play("blast"), initStartScreen()}
function initStartScreen() {// document.getElementById('moregame').style.display='none';
 ...
 for (var d = 0; 6 > d; d++) {var e = new Elements.Dot(assetLib.getData("dot"), assetLib.getData("pop"), d, d, canvas.width, canvas.height);
...
function initPreGame() {gameState = "tutorial";
 for (var a = 0; a < aTutorials.length; a++)
 if (aTutorials[a].gameNum == gameNum &&!aTutorials[a].shown) {background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height), userInput.addHitArea("continue", butEventHandler, null, {type: "rect",aRect: [100, 265, 220, 345]});
...
function butEventHandler(a, b) {switch (a) {case "langSelect":
 ...
 break;
 case "startGame":
 	if($("#playGameCount").val()>=5){alertInfos("text dayGametextuptext!");
 		return;}
 ...
 if (blockTap)
 break;
 if (b.isBeingDragged) {if (null == lastSelectedDot)
 break;
 ...
 aSelected[d].unSelect(), aSelected.splice(d, 1), d -= 1} else
 aDots[b.id].type < 6 && (aDots[b.id].select(), lastSelectedDot = aDots[b.id], aSelected = new Array(lastSelectedDot), playSound("drag"));
 break;
 case "mouseUp":
 if (!b.isDown) {if (blockTap)
 break;
 if (aSelected.length >= 3) {for (var e = 0, f = aSelected.length, g =!1, d = 0; f > d; d++) {if (7 == aSelected[d].type || 9 == aSelected[d].type) {g =!0;
 for (var h = 0; 6 > h; h++) {for (var i =!0, j = 0; j < aSelected.length; j++)
 aSelected[j] == aDots[6 * Math.floor(aSelected[d].id / 6) + h] && (i =!1);
 i && aSelected.push(aDots[6 * Math.floor(aSelected[d].id / 6) + h])}}
 if (8 == aSelected[d].type || 9 == aSelected[d].type) {g =!0;
 for (var h = 0; 7 > h; h++) {for (var i =!0, j = 0; j < aSelected.length; j++)
...
function sortThis(a) {a.sort(function(a, b) {var c = a.id, d = b.id;
 return d > c? -1: c > d? 1: 0})}
function updateScore(a) {gameScore += a, hud.updateScore(gameScore)}
function initGameOver() {// Game OvertextInitialize gametext
	addPlayGameCount();
 ...
 try {window.kaisergames.submitHighscore(0, gameScore);} catch (d) {}
...
function replaceDots() {for (var a = 0; a < aSelected.length; a++) {for (var b = 6; aSelected[a].id + b < 42 &&!aDots[aSelected[a].id + b].canHit;)
 b += 6;
...
function getYPosFromId(a) {return 62 * Math.floor(a / 6) + 110}
function getNewDotType() {return 9 * Math.random() < 8? Math.floor(6 * Math.random()): Math.floor(6 * Math.random() + 6)}
function updateGameEvent() {if (!manualPause &&!rotatePause && "game" == gameState) {var a = getDelta();
...
function updateGameOver() {if (!rotatePause && "gameOver" == gameState) {var a = getDelta();
...
function updateSplashScreenEvent() {if (!rotatePause && "splash" == gameState) {var a = getDelta();
...
function updateStartScreenEvent() {if (!rotatePause && "start" == gameState) {var a = getDelta();
...
function updateTutorialEvent() {if (!rotatePause && "tutorial" == gameState) {var a = getDelta();
...
function getDelta() {var a = (new Date).getTime(), b = (a - previousTime) / 1e3;
 return previousTime = a, b >.5 && (b = 0), b}
function renderSprite(a) {ctx.save(), ctx.translate(a.x, a.y), ctx.scale(a.scaleX, a.scaleY), ctx.rotate(a.rotation), a.render(ctx), ctx.restore()}
function checkSpriteCollision(a, b, c) {var d = a.x, e = a.y, f = b.x, g = b.y, h = (d - f) * (d - f) + (e - g) * (e - g), i = (a.radius + c) * (a.radius + c);
 return i > h?!0:!1}
...
function initLangSelect() {var a = preAssetLib.getData("langSelect");
 ...
 for (var b = 105, c = 0; c < aLangs.length; c++) {var d = canvas.width / 2 - b * aLangs.length / 2 + c * b, e = canvas.height / 2 - b / 2;
...
function playSound(a) {console.log("allowSound==" + allowSound + "a==" + a);
 allowSound && sound.play(a)}
function toggleMute() {muted =!muted, console.log("text==" + muted),allowSound && (muted? (music.pause(),sound.pause()): (sound.play(),music.play("blast"))), renderMuteBut()}
function renderMuteBut() {if (allowSound) {var a = assetLib.getData("muteBut"), b = 0;
 muted && (b = 1);
...
function rotatePauseOn() {rotatePause =!0, ctx.drawImage(assetLib.getImg("rotateDeviceMessage"), 0, 0), userInput.pauseIsOn =!0, pauseCoreOn()}
function rotatePauseOff() {rotatePause =!1, pauseCoreOff()}
function pauseCoreOn() {switch (allowSound && Howler.mute(), gameState) {case "start":
 break;
 case "help":
 break;
 case "game":
 ...
 break;
 case "end":}}
function pauseCoreOff() {switch (allowSound && (muted || Howler.unmute()), previousTime = (new Date).getTime(), userInput.pauseIsOn =!1, gameState) {case "splash":
...
var Utils;!function(a) {var b = function() {function a(a, b, c, d, e, f) {"undefined" == typeof f && (f =!0), this.oAssetData = {}, this.assetsLoaded = 0, this.totalAssets = b.length, this.ctx = c, this.canvasWidth = d, this.canvasHeight = e, this.showBar = f, this.topLeftX = this.canvasWidth / 2 - d / 4, this.topLeftY = this.canvasHeight / 2, this.showBar && (ctx.strokeStyle = "#aaaaaa", ctx.lineWidth = 2, ctx.fillStyle = "#ffffff", ctx.moveTo(this.topLeftX, this.topLeftY), ctx.lineTo(this.topLeftX + d / 2, this.topLeftY + 0), ctx.lineTo(this.topLeftX + d / 2, this.topLeftY + 20), ctx.lineTo(this.topLeftX + 0, this.topLeftY + 20), ctx.lineTo(this.topLeftX + 0, this.topLeftY + 0), ctx.stroke());
 ...
 this.loadImage(b[g])}
 return a.prototype.loadImage = function(a) {var b = this, c = new Image;
 c.onload = function() {b.oAssetData[a.id] = {}, b.oAssetData[a.id].img = c, void 0!= a.oData && (b.oAssetData[a.id].oData = a.oData), ++b.assetsLoaded, b.showBar && ctx.fillRect(b.topLeftX + 2, b.topLeftY + 2, (b.canvasWidth / 2 - 4) / b.totalAssets * b.assetsLoaded, 16), b.checkLoadComplete()}, c.src = a.file}, a.prototype.checkLoadComplete = function() {this.assetsLoaded == this.totalAssets && this.loadedCallback()}, a.prototype.onReady = function(a) {this.loadedCallback = a}, a.prototype.getImg = function(a) {return this.oAssetData[a].img}, a.prototype.getData = function(a) {return this.oAssetData[a]}, a}();
 a.AssetLoader = b}(Utils || (Utils = {}));
var Utils;!function(a) {var b = function() {function a(a, b, c, d) {this.x = 0, this.y = 0, this.rotation = 0, this.radius = 10, this.removeMe =!1, this.frameInc = 0, this.animType = "loop", this.offsetX = 0, this.offsetY = 0, this.scaleX = 1, this.scaleY = 1, this.oImgData = a, this.oAnims = this.oImgData.oData.oAnims, this.fps = b, this.radius = c, this.animId = d}
 return a.prototype.changeSpriteSheet = function(a) {this.oImgData = a, this.oAnims = this.oImgData.oData.oAnims, this.resetAnim()}, a.prototype.updateAnimation = function(a) {this.frameInc += this.fps * a}, a.prototype.resetAnim = function() {this.frameInc = 0}, a.prototype.setFrame = function(a) {this.fixedFrame = a}, a.prototype.setAnimType = function(a, b, c) {switch ("undefined" == typeof c && (c =!0), this.animId = b, this.animType = a, c && this.resetAnim(), a) {case "loop":
 break;
 case "once":
 this.maxIdx = this.oAnims[this.animId].length - 1}}, a.prototype.render = function(a) {if (null!= this.animId) {var b = this.oAnims[this.animId].length, c = Math.floor(this.frameInc), d = this.oAnims[this.animId][c % b], e = d * this.oImgData.oData.spriteWidth % this.oImgData.img.width, f = Math.floor(d / (this.oImgData.img.width / this.oImgData.oData.spriteWidth)) * this.oImgData.oData.spriteHeight;
 if ("once" == this.animType && c > this.maxIdx) {this.fixedFrame = this.oAnims[this.animId][b - 1], this.animId = null, this.animEndedFunc();
 var e = this.fixedFrame * this.oImgData.oData.spriteWidth % this.oImgData.img.width, f = Math.floor(this.fixedFrame / (this.oImgData.img.width / this.oImgData.oData.spriteWidth)) * this.oImgData.oData.spriteHeight}} else
 ...
 a.drawImage(this.oImgData.img, e, f, this.oImgData.oData.spriteWidth, this.oImgData.oData.spriteHeight, -this.oImgData.oData.spriteWidth / 2 + this.offsetX, -this.oImgData.oData.spriteHeight / 2 + this.offsetY, this.oImgData.oData.spriteWidth, this.oImgData.oData.spriteHeight)}, a}();
 a.AnimSprite = b}(Utils || (Utils = {}));
var Utils;!function(a) {var b = function() {function a(a, b) {this.x = 0, this.y = 0, this.rotation = 0, this.radius = 10, this.removeMe =!1, this.offsetX = 0, this.offsetY = 0, this.scaleX = 1, this.scaleY = 1, this.oImgData = a, this.radius = b}
 return a.prototype.setFrame = function(a) {this.frameNum = a}, a.prototype.render = function(a) {var b = this.frameNum * this.oImgData.oData.spriteWidth % this.oImgData.img.width, c = Math.floor(this.frameNum / (this.oImgData.img.width / this.oImgData.oData.spriteWidth)) * this.oImgData.oData.spriteHeight;
 a.drawImage(this.oImgData.img, b, c, this.oImgData.oData.spriteWidth, this.oImgData.oData.spriteHeight, -this.oImgData.oData.spriteWidth / 2 + this.offsetX, -this.oImgData.oData.spriteHeight / 2 + this.offsetY, this.oImgData.oData.spriteWidth, this.oImgData.oData.spriteHeight)}, a}();
 a.BasicSprite = b}(Utils || (Utils = {}));
var Utils;!function(a) {var b = function() {function a(a, b) {var c = this;
 this.canvasX = 0, this.canvasY = 0, this.canvasScaleX = 1, this.canvasScaleY = 1, this.prevHitTime = 0, this.pauseIsOn =!1, this.isDown =!1, this.isDetectingKeys =!1, this.isBugBrowser = b, a.addEventListener("touchstart", function(a) {for (var b = 0; b < a.changedTouches.length; b++)
 c.hitDown(a, a.changedTouches[b].pageX, a.changedTouches[b].pageY, a.changedTouches[b].identifier)},!1), a.addEventListener("touchend", function(a) {for (var b = 0; b < a.changedTouches.length; b++)
 c.hitUp(a, a.changedTouches[b].pageX, a.changedTouches[b].pageY, a.changedTouches[b].identifier)},!1), a.addEventListener("touchleave", function(a) {for (var b = 0; b < a.changedTouches.length; b++)
 c.leave(a, a.changedTouches[b].pageX, a.changedTouches[b].pageY, a.changedTouches[b].identifier)},!1), a.addEventListener("touchmove", function(a) {for (var b = 0; b < c.aHitAreas.length; b++)
 void 0!= a.changedTouches[b] && c.move(a, a.changedTouches[b].pageX, a.changedTouches[b].pageY, a.changedTouches[b].identifier,!0)},!1), a.addEventListener("mousedown", function(a) {c.isDown =!0, c.hitDown(a, a.pageX, a.pageY, 1)},!1), a.addEventListener("mouseup", function(a) {c.isDown =!1, c.hitUp(a, a.pageX, a.pageY, 1)},!1), a.addEventListener("mouseleave", function(a) {c.leave(a, a.pageX, a.pageY, 1)},!1), a.addEventListener("mousemove", function(a) {c.move(a, a.pageX, a.pageY, 1, c.isDown)},!1), this.aHitAreas = new Array, this.aKeys = new Array}
 return a.prototype.setCanvas = function(a, b, c, d) {this.canvasX = a, this.canvasY = b, this.canvasScaleX = c, this.canvasScaleY = d}, a.prototype.hitDown = function(a, b, c, d) {if (!this.pauseIsOn) {var e = (new Date).getTime();
 if (!(e - this.prevHitTime < 500)) {this.prevHitTime = e, a.preventDefault(), a.stopPropagation(), b = (b - this.canvasX) * this.canvasScaleX, c = (c - this.canvasY) * this.canvasScaleY;
 for (var f = 0; f < this.aHitAreas.length; f++)
 if (this.aHitAreas[f].rect && b > this.aHitAreas[f].area[0] && c > this.aHitAreas[f].area[1] && b < this.aHitAreas[f].area[2] && c < this.aHitAreas[f].area[3]) {this.aHitAreas[f].aTouchIdentifiers.push(d), this.aHitAreas[f].oData.isDown || (this.aHitAreas[f].oData.isDown =!0, this.aHitAreas[f].callback(this.aHitAreas[f].id, this.aHitAreas[f].oData));
 break}}}}, a.prototype.hitUp = function(a, b, c, d) {if (!this.pauseIsOn) {a.preventDefault(), a.stopPropagation(), b = (b - this.canvasX) * this.canvasScaleX, c = (c - this.canvasY) * this.canvasScaleY;
 for (var e = 0; e < this.aHitAreas.length; e++)
 if (this.aHitAreas[e].rect && b > this.aHitAreas[e].area[0] && c > this.aHitAreas[e].area[1] && b < this.aHitAreas[e].area[2] && c < this.aHitAreas[e].area[3]) {for (var f = 0; f < this.aHitAreas[e].aTouchIdentifiers.length; f++)
 this.aHitAreas[e].aTouchIdentifiers[f] == d && (this.aHitAreas[e].aTouchIdentifiers.splice(f, 1), f -= 1);
 0 == this.aHitAreas[e].aTouchIdentifiers.length && (this.aHitAreas[e].oData.isDown =!1, this.aHitAreas[e].oData.multiTouch && this.aHitAreas[e].callback(this.aHitAreas[e].id, this.aHitAreas[e].oData))}}}, a.prototype.move = function(a, b, c, d, e) {if (!this.pauseIsOn && e) {b = (b - this.canvasX) * this.canvasScaleX, c = (c - this.canvasY) * this.canvasScaleY;
 for (var f = 0; f < this.aHitAreas.length; f++)
 ...
 else if (this.aHitAreas[f].oData.isDown) {for (var g = 0; g < this.aHitAreas[f].aTouchIdentifiers.length; g++)
 this.aHitAreas[f].aTouchIdentifiers[g] == d && (this.aHitAreas[f].aTouchIdentifiers.splice(g, 1), g -= 1);
 0 == this.aHitAreas[f].aTouchIdentifiers.length && (this.aHitAreas[f].oData.isDown =!1, this.aHitAreas[f].oData.multiTouch && this.aHitAreas[f].callback(this.aHitAreas[f].id, this.aHitAreas[f].oData))}}}, a.prototype.leave = function(a, b, c, d) {if (!this.pauseIsOn) {a.preventDefault(), a.stopPropagation(), this.isDown =!1;
 for (var e = 0; e < this.aHitAreas.length; e++)
 if (this.aHitAreas[e].aTouchIdentifiers.length > 0) {for (var f = 0; f < this.aHitAreas[e].aTouchIdentifiers.length; f++)
 this.aHitAreas[e].aTouchIdentifiers[f] == d && (this.aHitAreas[e].aTouchIdentifiers.splice(f, 1), f -= 1);
 ...
 break}}}, a.prototype.keyDown = function(a) {for (var b = 0; b < this.aKeys.length; b++)
 a.keyCode == this.aKeys[b].keyCode && (this.aKeys[b].oData.isDown =!0, this.aKeys[b].callback(this.aKeys[b].id, this.aKeys[b].oData))}, a.prototype.keyUp = function(a) {for (var b = 0; b < this.aKeys.length; b++)
 a.keyCode == this.aKeys[b].keyCode && (this.aKeys[b].oData.isDown =!1, this.aKeys[b].callback(this.aKeys[b].id, this.aKeys[b].oData))}, a.prototype.addKey = function(a, b, c, d) {var e = this;
 this.isDetectingKeys || (window.addEventListener("keydown", function(a) {e.keyDown(a)},!1), window.addEventListener("keyup", function(a) {e.keyUp(a)},!1), this.isDetectingKeys =!0), null == c && (c = new Object), this.aKeys.push({id: a,callback: b,oData: c,keyCode: d})}, a.prototype.removeKey = function(a) {for (var b = 0; b < this.aKeys.length; b++)
 this.aKeys[b].id == a && (this.aKeys.splice(b, 1), b -= 1)}, a.prototype.addHitArea = function(a, b, c, d, e) {"undefined" == typeof e && (e =!1), null == c && (c = new Object), e && this.removeHitArea(a);
 var f = new Array;
 switch (d.type) {case "image":
 ...
 break;
 case "rect":
 this.aHitAreas.push({id: a,aTouchIdentifiers: f,callback: b,oData: c,rect:!0,area: d.aRect})}}, a.prototype.removeHitArea = function(a) {for (var b = 0; b < this.aHitAreas.length; b++)
 this.aHitAreas[b].id == a && (this.aHitAreas.splice(b, 1), b -= 1)}, a}();
 a.UserInput = b}(Utils || (Utils = {}));
var Utils;!function(a) {var b = function() {function a(a) {this.updateFreq = 10, this.updateInc = 0, this.frameAverage = 0, this.display = 1, this.log = "", this.render = function(a) {this.frameAverage += this.delta / this.updateFreq, ++this.updateInc >= this.updateFreq && (this.updateInc = 0, this.display = this.frameAverage, this.frameAverage = 0), a.textAlign = "left", ctx.font = "10px Helvetica", a.fillStyle = "#333333", a.beginPath(), a.rect(0, this.canvasHeight - 15, 40, 15), a.closePath(), a.fill(), a.fillStyle = "#ffffff", a.fillText(Math.round(1e3 / (1e3 * this.display)) + " fps " + this.log, 5, this.canvasHeight - 5)}, this.canvasHeight = a}
 return a.prototype.update = function(a) {this.delta = a}, a}();
 a.FpsMeter = b}(Utils || (Utils = {}));
var Elements;!function(a) {var b = function() {function a(a, b, c) {this.x = 0, this.y = 0, this.targY = 0, this.incY = 0, this.oImgData = a, this.canvasWidth = b, this.canvasHeight = c}
 return a.prototype.startScroll = function(a) {this.targY -= 5 * a, TweenLite.killTweensOf(this), TweenLite.to(this, 4, {y: this.targY,ease: "Quad.easeOut"})}, a.prototype.updateScroll = function(a) {this.incY += 5 * a, this.x = this.x - 50 * Math.sin(this.incY / 10) * a, this.y = this.y - 50 * a}, a.prototype.render = function(a) {this.x = this.x % this.canvasWidth, this.y = this.y % this.canvasHeight, this.x < 0 && (this.x += this.canvasWidth), this.y < 0 && (this.y += this.canvasHeight), a.drawImage(this.oImgData.img, this.x, this.y, this.canvasWidth, this.canvasHeight, 0, 0, this.canvasWidth, this.canvasHeight)}, a}();
 a.Background = b}(Elements || (Elements = {}));
var Elements;!function(a) {var b = function() {function a(a, b, c, d, e) {this.incY = 0, this.score = 0, this.highestScore = 0, this.posY = 0, this.bgX = 0, this.bgY = 0, this.atmosX = 0, this.atmosY = 0, this.oButs = a, this.oBgImgData = b, this.oTitleImgData = c, this.canvasWidth = d, this.canvasHeight = e, this.posY = -this.canvasHeight}
 return a.prototype.setRenderFunc = function(a) {switch (this.posY = -this.canvasHeight, a) {case "start":
 this.renderFunc = this.renderStartScreen, TweenLite.to(this,.5, {posY: 0});
 break;
 case "help":
 break;
 case "end":}}, a.prototype.render = function(a, b) {this.renderFunc(a, b)}, a.prototype.renderStartScreen = function(a, b) {this.incY += 5 * b, this.bgY -= 50 * b, this.bgX -= 50 * Math.sin(this.incY / 10) * b;
 ...
 a.Screens = b}(Elements || (Elements = {}));
var Elements;!function(a) {var b = function() {function a(a, b, c) {this.inc = 0, this.oSplashScreenImgData = a, this.canvasWidth = b, this.canvasHeight = c, this.posY = -this.canvasHeight, TweenLite.to(this,.5, {posY: 0})}
 return a.prototype.render = function(a, b) {this.inc += 5 * b, a.drawImage(this.oSplashScreenImgData.img, 0, 0 - this.posY)}, a}();
 a.Splash = b}(Elements || (Elements = {}));
var Elements;!function(a) {var b = function() {function a(a, b, c, d, e, f, g) {this.timer =.3, this.endTime = 0, this.posY = 0, this.numberSpace = 17, this.incY = 0, this.oPanelsImgData = a, this.oThinNumbersImgData = b, this.oFatNumbersImgData = c, this.panelType = d, this.aButs = e, this.canvasWidth = f, this.canvasHeight = g}
 return a.prototype.update = function(a) {this.incY += 5 * a}, a.prototype.startTween = function() {this.posY = 550, TweenLite.to(this,.8, {posY: 0,ease: "Back.easeOut"})}, a.prototype.render = function(a) {switch (this.panelType) {case "start":
 ...
 for (var e = this.oScoreData.gameScore, f = 0; f < e.toString().length; f++) {b = parseFloat(e.toString().charAt(f));
 ...
 for (var e = this.oScoreData.longestChain, f = 0; f < e.toString().length; f++) {b = parseFloat(e.toString().charAt(f));
 ...
 for (var e = this.oScoreData.balloonsPopped, f = 0; f < e.toString().length; f++) {b = parseFloat(e.toString().charAt(f));
 var c = b * this.oThinNumbersImgData.oData.spriteWidth % this.oThinNumbersImgData.img.width, d = Math.floor(b / (this.oThinNumbersImgData.img.width / this.oThinNumbersImgData.oData.spriteWidth)) * this.oThinNumbersImgData.oData.spriteHeight;
 ...
 for (var e = this.oScoreData.highScore, f = 0; f < e.toString().length; f++) {b = parseFloat(e.toString().charAt(f));
 var c = b * this.oThinNumbersImgData.oData.spriteWidth % this.oThinNumbersImgData.img.width, d = Math.floor(b / (this.oThinNumbersImgData.img.width / this.oThinNumbersImgData.oData.spriteWidth)) * this.oThinNumbersImgData.oData.spriteHeight;
 ...
 a.Panel = b}(Elements || (Elements = {}));
var Elements;!function(a) {var b = function() {function a(a, b, c, d, e) {this.score = 0, this.oHudImgData = a, this.oThinNumbersImgData = b, this.oFatNumbersImgData = c, this.canvasWidth = d, this.canvasHeight = e, this.score = 0}
 return a.prototype.render = function(a, b) {a.drawImage(this.oHudImgData.img, 0, 0);
 for (var c = 0; c < this.score.toString().length; c++) {var d = parseFloat(this.score.toString().charAt(c)), e = d * this.oFatNumbersImgData.oData.spriteWidth % this.oFatNumbersImgData.img.width, f = Math.floor(d / (this.oFatNumbersImgData.img.width / this.oFatNumbersImgData.oData.spriteWidth)) * this.oFatNumbersImgData.oData.spriteHeight;
 ...
 for (var c = 0; c < b.toString().length; c++) {var d = parseFloat(b.toString().charAt(c)), e = d * this.oThinNumbersImgData.oData.spriteWidth % this.oThinNumbersImgData.img.width, f = Math.floor(d / (this.oThinNumbersImgData.img.width / this.oThinNumbersImgData.oData.spriteWidth)) * this.oThinNumbersImgData.oData.spriteHeight;
 a.drawImage(this.oThinNumbersImgData.img, e, f, this.oThinNumbersImgData.oData.spriteWidth, this.oThinNumbersImgData.oData.spriteHeight, 31 + 17 * c - 17 * b.toString().length / 2, 22, this.oThinNumbersImgData.oData.spriteWidth, this.oThinNumbersImgData.oData.spriteHeight)}}, a.prototype.updateScore = function(a) {this.score = a}, a}();
 a.Hud = b}(Elements || (Elements = {}));
var __extends = this.__extends || function(a, b) {function c() {this.constructor = a}
 c.prototype = b.prototype, a.prototype = new c}, Elements;!function(a) {var b = function(a) {function b(b, c, d, e, f, g) {a.call(this, b, 15, 15, "unSelected" + e), this.canHit =!0, this.selected =!1, this.popDelay = -1, this.oDotImgData = b, this.oPopImgData = c, this.id = d, this.type = this.colour = e, this.canvasWidth = f, this.canvasHeight = g, this.offsetY = 10, this.frameInc = Math.ceil(20 * Math.random()), this.animEndedFunc = this.burst}
 return __extends(b, a), b.prototype.fall = function() {this.y = this.targY + 550, this.x = this.targX, TweenLite.to(this,.5, {y: this.targY,delay:.02 * this.id,ease: "Quad.easeOut"})}, b.prototype.replaceWithExistingDot = function(a) {this.y = a.targY, this.type = a.type, this.removeMe =!1, this.canHit =!0, this.x = this.targX, this.changeSpriteSheet(this.oDotImgData), this.frameInc = a.frameInc, this.unSelect(), TweenLite.to(this,.5, {y: this.targY,ease: "Quad.easeInOut"})}, b.prototype.replaceWithNewDot = function(a) {this.y = this.targY + 550, this.type = a, this.removeMe =!1, this.canHit =!0, this.x = this.targX, this.changeSpriteSheet(this.oDotImgData), this.frameInc = Math.ceil(20 * Math.random()), this.unSelect(), TweenLite.to(this,.5, {y: this.targY,ease: "Quad.easeOut"})}, b.prototype.trySelect = function(a) {var b = a.x - this.x, c = a.targY - this.targY, d = b * b + c * c;
 return 8100 > d && (a.colour == this.colour || this.type >= 6 || 6 == a.type)? (this.select(), this.type >= 7 && (this.colour = a.colour), this.prevDot = a, this): a}, b.prototype.select = function() {this.selected || (this.setAnimType("loop", "selected" + this.type,!1), this.selected =!0)}, b.prototype.unSelect = function() {this.setAnimType("loop", "unSelected" + this.type,!1), this.selected =!1, this.colour = this.type}, b.prototype.update = function(b) {a.prototype.updateAnimation.call(this, b), this.popDelay >= 0 && (this.popDelay += b, this.popDelay > this.popDelayTarg && this.pop())}, b.prototype.hit = function(a) {this.popDelay = 0, this.popDelayTarg =.25 * a}, b.prototype.pop = function() {this.popDelay = -1, this.changeSpriteSheet(this.oPopImgData), this.canHit =!1, this.setAnimType("once", "pop")}, b.prototype.burst = function() {this.removeMe =!0}, b}(Utils.AnimSprite);
 a.Dot = b}(Elements || (Elements = {}));
var requestAnimFrame = function() {return window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function(a) {window.setTimeout(a, 1e3 / 60, (new Date).getTime())}}(), previousTime, canvas = document.getElementById("canvas"), ctx = canvas.getContext("2d");
...
resizeCanvas(), window.onresize = function() {setTimeout(function() {resizeCanvas()}, 1)}, window.addEventListener("load", function() {setTimeout(function() {resizeCanvas()}, 0), window.addEventListener("orientationchange", function() {resizeCanvas()},!1)}), (allowSound =!0, sound = new Howl({urls: ["audio/sound.mp3"],sprite: {pop1: [0, 400],pop2: [500, 400],pop3: [1e3, 750],click: [2e3, 300],timesUp: [2500, 2400],explode: [5e3, 1500],drag: [7e3, 400]}}), music = new Howl({urls: ["audio/music.mp3"], sprite: {blast: [0, 31000]},volume:.25,loop:!1}));
...
function compareTime(firstDate, secondDate) {var strNow = firstDate.getFullYear() + "-" + (firstDate.getMonth() + 1) + "-" + firstDate.getDate();
	var signInTime = secondDate.getFullYear() + "-" + (secondDate.getMonth() + 1) + "-" + secondDate.getDate();
	if (strNow == signInTime) {return true;} else {return false;}}

function isduihuanlove(highScore){if(highScore<5000){return;}
	$.ajax({type: "POST",
		url: "../../MSActivityServlet",
		data: "method=getUser",
		dataType: "json",
		success: function(data) {if(data.gameTime){var now = new Date();
				var playGameLove = new Date(data.gameTime);
				if (!compareTime(now, playGameLove)&&$("#playGameCount").val()<=5) {var conetent="text!texthighscore:"+highScore+"text"+(5-data.playGameCount)+"textGametext, text?";
					duihuanlove(highScore,conetent);}}else{if ($("#playGameCount").val()<=5) {var conetent="text!texthighscore:"+highScore+"text"+(5-data.playGameCount)+"textGametext, text?";
					duihuanlove(highScore,conetent);}}}});}

function duihuanlove(highScore,content){alertInfosConfirm(content, goToDuihuan, cancelAction,highScore);}
function goToDuihuan(highScore){$.ajax({type: "POST",
			url: "../../MSActivityServlet",
			data: "method=playGameAddLove" +"&score="+highScore,
			dataType: "json",
			success: function(data) {if(data.resultCode==0){alertInfos("text"+data.resultMsg+"text");}}});}
function addPlayGameCount(){$.ajax({type: "POST",
			url: "../../MSActivityServlet",
			data: "method=addPlayGameCount",
			dataType: "json",
			success: function(data) {if(data.resultCode==0){$("#playGameCount").val(data.resultMsg);}}});}
// JavaScript Document
// defineBeautifiertextfunction
var DialogFx = function(ele, opt) {this.$element = ele,
 this.defaults = {'id': 'somedialog',
		'dialogFx': '0',},
 this.options = $.extend({}, this.defaults, opt)}
// defineDialogFxtextshowmethod
DialogFx.prototype = {Show: function() {$("#"+this.options.id).removeClass("dialog--close");
		$("#"+this.options.id).addClass("dialog--open");},
	Hidden: function() {$("#"+this.options.id).removeClass("dialog--open");
		$("#"+this.options.id).removeClass("dialog--close");}}
// textmiddletextDialogShowobject
$.fn.DialogShow = function(options) {// createBeautifiertext
 var dialogFx = new DialogFx(this, options);
	if(options.dialogFx == "0"){return dialogFx.Hidden();} else {// calltextmethod
 	return dialogFx.Show();}}

function alertInfos(content) {var th = '<tr><td>' + content + '</td></tr>';
	$("#excodeTbalert").html(th);
	$(this).DialogShow({'id': 'somedialog_alert',
		'dialogFx': '1',});
	$("#alertInfoId").unbind('click');
	$("#alertInfoId").click(function() {window.location.href="../../CMBSActivity/CMBSThanksGiving.html?parentUserId="+$("#parentUserId").val()+"&id="+$("#userId").val();
		$(this).DialogShow({'id': 'somedialog_alert',
			'dialogFx': '0',});})}
...
function alertInfosConfirm(content, confirm, cancel,highScore) {var th = '<tr><td>' + content + '</td></tr>';
	$("#excodeTbalertconfirm").html(th);
	$(this).DialogShow({'id': 'somedialog_alert_confirm',
		'dialogFx': '1',});
	$("#alertInfoIdconfirm").unbind('click');
	$("#alertInfoIdconfirm").click(function() {if(highScore){confirm(highScore);}else{confirm();}
		$(this).DialogShow({'id': 'somedialog_alert_confirm',
			'dialogFx': '0',});})
	$("#alertInfoIdcancel").unbind('click');
	$("#alertInfoIdcancel").click(function() {cancel();
		$(this).DialogShow({'id': 'somedialog_alert_confirm',
			'dialogFx': '0',});})}