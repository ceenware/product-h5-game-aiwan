...
// Start Game
start.onclick=function(){// text
	this.remove();
	// text
	var proLeft=0;
	var timer_pro=setInterval(function(){proLeft-=0.045;
		pros.style.backgroundPositionX=proLeft+'px';
		if (proLeft<=-270) {clearInterval(timer_pro);
			clearInterval(circle);
			over.style.display='block';
			restart.style.display='block';}},5)
	star();	// Level textGametextfunction
	res();	// callRestarttextfunction}

// game startfunction
function star(){//=============================================================================Gametext row text
	circle=setInterval(function(){//text
	var arrPos = [{left:"170px",top:"210px"},
		{left:"50px",top:"280px"},
		{left:"45px",top:"370px"},
		{left:"70px",top:"480px"},
	...
	var wfLocation=rand(0,8);	// text
	newImg.style.left=arrPos[wfLocation].left;
	newImg.style.top=arrPos[wfLocation].top;
	newImg.style.position='relative';
	var X=rand(0,2);		// textsmall text
	if (X<2){X='h';}else{X='x';}
	var y=0;
		newImg.style.display='block';
		var appear0=setInterval(function(){newImg.src='img/'+X+y+'.png';
			y++;
			var indexs=0;
			newImg.onclick=function(){indexs++;
				if (indexs>1) {return false;		// textpoint text1text textpoint}
				y=5;
				for (var i=0;i<4;i++) {y++;
					newImg.src=wolf_1[y];
					if(y>9){y--;}}
				if (X=='h') {s+=10;
					scores.innerHTML=s;}else if (X=='x'){s-=10;
					if (s<=0) {s=0;}
					scores.innerHTML=s;}}
			if (y>5) {clearInterval(appear0);
				setTimeout(function(){y=5;
					var appear1=setInterval(function(){newImg.src='img/'+X+y+'.png';
						console.log(y);
						y--;
						if (y<0) {clearInterval(appear1);
							// newImg.style.display='none';
							newImg.remove();}},50)},stay)}},50);},secs)
	s=0;
	scores.innerHTML=s;
//=============================================================================Game Over}
// Restartfunction
function res(){restart.onclick=function(){// text
