(function(){//text
 let snake_move = undefined; //Movetext
 let when_used = undefined; //text
 let whenUseTime = 0; //text
 let startGamebool = true; //Start Gametext
 let startPauseBool = true; //PausetextContinue Gametext
 let speed = 200; //textspeed
 let gameBGImg = 'url(./images/gameBGImg1.png)';// text

 //textmethod
 let $ = (id) => {//getidtext point
 return document.getElementById(id);};

 let removeEleId = (removeELe) => {//deleteidtext point
 removeEle.parentNode.remove(removeELe)};

 let removeElementsByClass = (classname) => {// deleteclasstext point
 let elements = document.getElementsByClassName(classname);
 while(elements.length>0){elements[0].parentNode.removeChild(elements[0])}};

 //Gameset text
 let setGame = {speed: 200,
 ...
 for(let i =0;i<speedClass.length;i++){speedClass[i].onclick = () => {setGame.speed = speedClass[i].value;}}
 let bgClass = document.getElementsByClassName('gameBGImgRadio');
 bgClass[0].setAttribute('checked','true');
 for(let i =0;i<bgClass.length;i++){bgClass[i].onclick = () => {setGame.gameBGImg = bgClass[i].value;}}

 // defineSnaketext
 class RETRO_SNAKER {constructor(){//text
 ...
 //Score
 this.score = 0;}
 init(){this.numShow();//showScoretext}
 startGame(){this.food(); //createtext
 this.snake(); //createtext}
 numShow(){$('score').innerHTML = this.score;
 $('whenUsedTime').innerHTML = 0+'s';}
 food(){let food = document.createElement('div');
 ...
 snake(){for(let i = 0;i<this.snakeBody.length;i++){let snake = document.createElement('div');
 ...
 for(let i = 0;i<snakeEye.length;i++){snakeEye[i].style.width = this.snakeEyeWidth + 'px';
 ...
 break;}}
 move(){//text
 for(let i = this.snakeBody.length-1;i>0;i--){this.snakeBody[i][0] = this.snakeBody[i-1][0];
 ...
 default:
 break;}
 //deletetextpoint textRender
 removeElementsByClass('snake');
 this.snake();
 // textx ytext text
 if(this.snakeBody[0][0] == this.food_x && this.snakeBody[0][1] == this.food_y){let snakeTail_x = this.snakeBody[this.snakeBody.length-1][0];
 ...
 this.food();}
 // checktext
 if(this.snakeBody[0][1]<0 || this.snakeBody[0][1]>=this.mapHeight/this.snakeHeight){this.reloadGame();}
 if(this.snakeBody[0][0]<0 || this.snakeBody[0][0]>=this.mapWidth/this.snakeWidth){this.reloadGame();}
 //checktext
 let snakeHeader_x = this.snakeBody[0][0];
 let snakeHeader_y = this.snakeBody[0][1];
 for(let i = 1;i<this.snakeBody.length;i++){let snakeBody_x = this.snakeBody[i][0];
 let snakeBody_y = this.snakeBody[i][1];
 if(snakeHeader_x == snakeBody_x && snakeHeader_y == snakeBody_y){this.reloadGame();}}}
 setDirect(code){switch(code){case 37:
 if(this.left){this.direct = 'left';
 this.left = false;
 this.right = false;
 this.up = true;
 this.down = true;}
 ...
 break;
 case 39:
 if(this.right){this.direct = 'right';
 this.left = false;
 this.right = false;
 this.up = true;
 this.down = true;}
 ...
 default:
 break;}}
 reloadGame(){removeElementsByClass('snake');
 removeElementsByClass('food');
 clearInterval(snake_move);
 clearInterval(when_used);
 document.onkeydown = function (e) {e.returnValue = false;
 return false;};
 //text
 if(this.score > $('bestScore').innerHTML){$('bestScore').innerHTML = this.score;}
 //Initialize
 ...
 $('loserGetScore').innerHTML = this.score+'score';
 ...
 //text
 window.onload = () => {window.quitGame = () => {//textGame
 if (confirm("textClosetextSnakeGametext?")) {window.opener=null;
 window.open('','_self');
 window.close();}};

 window.enterGame = () => {//Enter Game
 ...
 setTimeout(()=> {$('main-start-interface').removeAttribute('style');
 $('main-start-interface').style.display = 'none';},2000)};

 //Gameset
 window.setGame = () => {//textmodal
 $('setGameModal').style.display = 'block';};
 window.sureSet = () => {//point text
 speed = setGame.speed;
 gameBGImg = setGame.gameBGImg;
 $('setGameModal').style.display = 'none';};
 window.cancelSet = () => {//point textCancel
 $('setGameModal').style.display = 'none';};
 ...
 document.onkeyup = (event) => {//Space
 let code = undefined;
 if(window.event) {code = window.event.keyCode;} else {code = event.keyCode;}
 switch(code){case 32:
 startAndPauseGame();
 break;
 default:
 ...
 window.startPause = () => {//Start GametextPause Game
 startAndPauseGame();};

 //Failedmodal
 window.cancelLoser = () => {$('loserModal').style.display = 'none';};
 window.playAgain = () => {$('loserModal').style.display = 'none';
 startAndPauseGame();};

 //StarttextPause Game text
 let startAndPauseGame = () => {if(startPauseBool){//Start
 if(startGamebool){retro_snaker.startGame();
 startGamebool = false;}
 $('startPause').setAttribute('src','./images/pause.png');
 //textkeytextMovetext
 snake_move = setInterval(()=>{retro_snaker.move();},speed);
 when_used = setInterval(()=>{whenUseTime += 1;
 $('whenUsedTime').innerHTML = whenUseTime+ 's';},1000);
 // textkey
 document.onkeydown = (event) => {//updownleftright
 let code = undefined;
 if(window.event) {code = window.event.keyCode;} else {code = event.keyCode;}
 retro_snaker.setDirect(code);};
 startPauseBool = false;}else{//Pause
 $('startPause').setAttribute('src','./images/start.png');
 clearInterval(snake_move);
 clearInterval(when_used);
 document.onkeydown = function (e) {e.returnValue = false;
