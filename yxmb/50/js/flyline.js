Main.flyLine = function(angle){var speedRate = 7;//speedtext
	var gRate = 100;//text
	var flyBird;
	...
	function clock(){tParam += 1/fps;
		moveBird(flySpeed, flyRad);
		// stage.update();}
	
	function stageMove(difDistance, totalDistance){Main.fire("moveStage", difDistance, totalDistance);}
	var offsetX = Main.penguinOffsetX;
	var offsetY = Main.height - Main.floorLine + 32;
	var flyStageDistance = Main.visibleDistance;
	var fadeCnt = 0;
	// var hitOffsetY = 100;//textDistance
	var hitOffsetY = Main.floorLine - Main.penguin.y - Main.penguinHeight / 2;
	var tween;
	function moveBird(speed, rad){var x = speed * Math.cos(rad) * tParam;
		var y = speed * Math.sin(rad) * tParam - 1/2 * g * square(tParam) + hitOffsetY;//v0*sina-1/2gt2
		var vy = speed * Math.sin(rad) - g * tParam;
		var vx = speed * Math.cos(rad);//xtextuptextspeed			
		var slope = -g / square(speed) / square(Math.cos(rad)) * x + Math.sin(rad) / Math.cos(rad);//text					
		flyBird.rotation = slope * angle;
		
		var dif = vx * 1/fps;
		if(x + startX > flyStageDistance){stageMove(dif, x + startX);//textMovetextDistance}else{flyBird.x = sWidth - offsetX - (x + startX);}
		//circle.x = (x + startX);
		//circle.y = y;
		flyBird.y = sHeight - offsetY - y;

		if(y <= 0 && slope < 0){//reset				
			tParam = 0;
			startX += x;
			startY = 0;
			...
			if(Math.abs(flyBird.rotation) >= 46 && fadeCnt == 1){//textlarge, text
				turnDown(flyBird.x, flyBird.y, "down");
				stopFly();
				gameOver(x + startX);
				return;}
			if(fadeCnt == 3){//end game
				stopFly();
				//addlong text
				var slideTime = Math.cos(rad) * 800;		
				addLongTrace(x, dif, slideTime);}else{tween = createjs.Tween.get(flyBird, {loop:false}).to({rotation:0}, 50, createjs.Ease.bounceOut).call(function(){startFly();
					createjs.Tween.removeTweens(tween);});
				//addtext
				addTrace(dif);}}}

	var longTracer;
	function addLongTrace(xPass, dif, timeC){longTracer = new createjs.Bitmap(Resource.get("img/longtraces.png"));
		...
		function longTracerClock(){longTracer.x += dif;
			stageMove(dif, xPass + startX + 452);}
		setTimeout(function(){turnDown(flyBird.x, flyBird.y, "slide");
 ...
 rect.graphics.endStroke();
	...
	function addTrace(dif){var totalD = 0;
		...
		function tracerClock(){tracer.x += dif;
			totalD += dif;
			if(totalD > sWidth * 2 / 3){Main.log("text");
				createjs.Ticker.removeEventListener("tick", tracerClock);
				stage.removeChild(tracer);}}}
	function turnDown(fx, fy, status){Main.stage.removeChild(flyBird);
	...
	function gameOver(distance){//todo
		
		Main.fire("gameOver", distance, flyBird.x);}
	Main.on("replay", function(){// text
		if(longTracer){Main.log("longTracerDisp")
			stage.removeChild(longTracer);}
		stage.removeChild(flyBird);
		flyBird = null;
		stopFly();});
	
	function stopFly(){createjs.Ticker.removeEventListener("tick", clock);}
	function startFly(){stopFly();
		clock(); // textLevel text
		createjs.Ticker.addEventListener("tick", clock);}
	
	//text
	function angleToRad(angle){return Math.PI / 180 * angle;}
	//²
