...
var Resource;;~function(res){// text
	res.load = function(){if(res.queue!= null) return;
		res.queue = new createjs.LoadQueue(false);
		
		res.queue.addEventListener("progress",function(e) {//console.log(e);// text: e.loaded
			btGame.gameLoading(e.loaded);});
		res.queue.addEventListener("complete",function(e) {Main.log("text", e);});
		res.queue.addEventListener("error",function(e) {btGame.gameLoading(0.1, e.text);});
	...
	res.get = function(path){return res.queue.getResult(path);}}(Resource || (Resource = {}));

Main.log = function(){Main.debug && console.log(arguments);}

Main.shareTextMap = {"0": {tip: "textmiddle,text!Try Again~",title: "textGame, text!"},"1": {tip: "text,text!!",title: "text${m}text, text${n}%person, text!"},"2": {tip: "text!text!",title: "text${m}text, text${n}%person, text!"},"3": {tip: "text~text!",title: "text${m}text, text${n}%person, text!"}};
Main.getShareText = function(distance){var i = 0, n = 0;
	if(distance > 0){i = 1;
	...
	btGame.setShare({title:share.title.replace("${m}", distance).replace("${n}", n)});
	return share;};


// game starttext
Main.startGame = function(){var container = new createjs.Container();
	...
	beginGame.addEventListener("click", function(){createjs.Tween.get(container, {loop: false}).to({alpha: 0}, 300).call(function(){Main.initGame();});});
	...
	moreGame.addEventListener("click", function(){clickMore();});
	container.addChild(moreGame);
	
	Main.stage.addChild(container);}

// Game Overtext
Main.endGame = function(distance){if(!Main.endContainer){Main.endContainer = new createjs.Container();}else{Main.endContainer.removeAllEventListeners();
	...
	// text
	var text1 = new createjs.Text("textDistance: "+distance+" M", "bold 28pt Tahoma Helvetica Arial sans-serif", "#ffffff");
	...
	text2.text = "textDistance: " + Main.maxScore + " M";
	text2.y = 248;
	...
	again.addEventListener("click", function(){createjs.Tween.get(container, {loop: false}).to({y: -Main.height, alpha: 0}, 500, createjs.Ease.quintInOut).call(function(){container.alpha = 1;
	...
	notify.addEventListener("click", function(){dp_share();});
	...
	more.alpha = 0.1;
	more.y = 870;
	more.addEventListener("click", function(){clickMore();});
	container.addChild(more);
	
	Main.stage.addChild(container);
	container.y = -Main.height;
	createjs.Tween.get(container, {loop: false}).to({y:0, alpha:1}, 500, createjs.Ease.quintInOut);
		dp_submitScore(distance);}

Main.initStage = function(){// text, textInitializetext
	if(!this.stage){Main.stage = new createjs.Stage("canvas");}
	...
	// text
	Resource.load();
	Resource.queue.addEventListener("complete", function(){// Main.startGame();
		Main.startGame();});};

Main.initGame = function(){Main.initGame = function(){}; // textInitialize
	Main.stage.enableMouseOver(true);
	Main.log("StartInitialize game");
	Main.log("Initialize: Gametext");
	...
	Main.log("Initialize: text");
	var bearImg = new createjs.Bitmap(Resource.get("img/bear.png"));
	var bearBound = bearImg.getBounds();
	var bearSS = new createjs.SpriteSheet({"framerate": 4,"animations":{"normal": [0, 0],"prepare": [1, 2, false, 1.5],"shoot": {frames: [1, 3, 4, 5],
				next: false,
				speed: 3},"shootNull":{frames: [1, 3, 4, 0],
				next: false,
				speed: 2}},
	...
	if(Main.debug){Main.log("textpoint:" + this.knockPoint);
	...
	Main.log("Initialize: Score");
	var score = new createjs.Container();
	score.setBounds(0, 0, 238, 56);
	
	if(this.debug){var shape = new createjs.Shape();
		shape.graphics.beginFill("#fefefe").drawRect(0, 0, score.getBounds().width, score.getBounds().height);
	...
	var penguinSS = new createjs.SpriteSheet({"framerate": 5,"animations": {"normal": {frames:[0, 1, 0, 1, 2],next: false},"nod": [2, 2, "normal"],"jump": {frames: [2, 3, 4, 5],speed: 2,next: false}},"images": [Resource.get("img/penguin.png")],"frames": {width: 180,
			height: 170,
			count: 6}});
	...
	Main.log("Initialize:Game Overtext");
	var endData = {images: [Resource.get("img/flyend.png")],
	...
	// textScorecard
	Main.log("Initialize:Game OvertextScorecard");
	...
	// Bind events, htc textspeed
	setTimeout(function(){Main.initEvent();}, 200);};

Main.gameOverSocreHd = {over: function(event){console.log(this.hover);},out: function(event){console.log(this.hover);}}

// text
Main.penguinOffsetX = 260;
Main.reset = {replay: function(){this.guide("100M");
		this.score();
		this.penguin();
		this.bear();
		this.background();
		...
		Main.isPlaying = false; // textrowGame
		Main.isQuiver = false;
		
		Main.guideMoveLength = 0;
		Main.guideMoveIndex = 1;
		
		Main.fire("replay");},guide: function(text){Main.guide.x = -Main.guideWidth;
		var guideText = Main.guideText;
		if(text){guideText.text = text;
			guideText.x = (Main.guideWidth - guideText.getMeasuredWidth()) / 2;}},score: function(txt){var text = Main.scoreText;
		var topWidth = Main.scoreTop.getMeasuredWidth(); // top textwidth
		var maxWidth = Main.score.getBounds().width - topWidth;
		
		text.text = txt || (Main.maxScore + "M");
		var width = text.getMeasuredWidth();
		if(width >= maxWidth){text.x = topWidth;
			text.text = text.text.slice(0, -1);}else{text.x = topWidth + (maxWidth - width) / 2;}
		Main.score.x = 68;
		Main.score.y = 681;},gameOverScore: function(txt){var text = Main.gameOverScoreText;
		var offsetX = 11, width = 100;
		if(txt){text.text = txt;
			var textWidth = text.getMeasuredWidth();
			if(textWidth >= width){text.x = offsetX;}else{text.x = (width - textWidth) / 2 + offsetX;}}
		Main.gameOverScore.y = -Main.gameOverScore.getBounds().height;},penguin: function(){var penguin = Main.penguin;
		var bound = Main.penguin.getBounds();
		penguin.x = Main.width - Main.penguinOffsetX;
		penguin.y = 0;
		penguin.alpha = 1;
		penguin.rotation = 0;
		penguin.gotoAndPlay("normal");},bear: function(){var bear = Main.bear;
		var bound = bear.getBounds();
		bear.x = Main.width - 330;
		bear.y = 500;
		bear.gotoAndPlay("normal");},background: function(){var background = Main.background;
		Main.stopMoveStage();
		background.regX = 2 * Main.width;},endPenguin: function(visible){if(visible){if(!Main.gameOverPenguin){Main.gameOverPenguin = new createjs.Sprite(Main.gameOverPenguinSS);
			...
			Main.gameOverPenguin.y = Main.penguin.y;}else{Main.penguin.alpha = 1;
			Main.gameOverPenguin && (Main.gameOverPenguin.alpha = 0);}}};


Main.initEvent = function(){Main.clickTime = 0;
...
var eventCD = false;
Main.eventHandler = {stageClick: function(event){// textpoint textGame Overtext
		// text, text
		if(!eventCD){Main.fire("stageClick", Main.clickTime);
			eventCD = true;
			setTimeout(function(){eventCD = false;}, 500);}}};


Main.penguinAnimation = null;
Main.isPlaying = false; // textrowGame
Main.isQuiver = false; // textmiddle
/* text */
Main.on("stageClick", function(clickTime){// Level textpoint text: textdowntext + white textReadytext
	if(!Main.isPlaying){Main.isPlaying = true;
		// text
		this.reset.penguin();
		this.reset.bear();
		...
		Main.penguinAnimation.to({y: toY}, Main.fallingTime, createjs.Ease.quadIn).call(function(){Main.fire("gameOver", 0);
							Main.log("textdowntext");});		
		
		// text
		Main.bearListenerShoot();}else if(!Main.penguinAnimation._paused){// text, text pause textmethod..
		...
		// textmiddletext?
		if(Main.isQuiver){clickX = Math.ceil(clickX / this.penguinHeight * 100);
			var angle = Main.calculateAngle(clickX);
			Main.log("textmiddletext:" + angle);
			Main.fire("knockPenguin", angle);
			// text
			Main.bearRemoveListenerShoot();}else{Main.log("textmiddle!!");}
		this.bear.gotoAndPlay("shoot");}});

Main.bearListenerShoot = function(){this.bearRemoveListenerShoot();
	this.bear.addEventListener("animationend", Main.bearShoot);}
Main.bearRemoveListenerShoot = function(){this.bear.removeEventListener("animationend", Main.bearShoot);}

// textmiddle, textReady
Main.bearShoot = function(){if(!Main.isQuiver && Main.bear.currentAnimation == "shoot"){Main.bear.gotoAndPlay("prepare");}}

Main.on("knockPenguin", function(event, angle){Main.penguinAnimation.setPaused(true);
	Main.flyLine(angle);});


Main.on("moveStage", function(event, distance, totalDistance){Main.moveStage(distance, totalDistance);});

Main._moveStage = function(distance, totalDistance){Main.moveStage = Main.moveStage2;
	Main.moveStage(distance, totalDistance);}
Main.moveStage = Main._moveStage;
Main.stopMoveStage = function(){// this.stage.removeEventListener("tick", testMoveStage);
	Main.moveStage = Main._moveStage;}

// 2, text, text, text, Main.visibleDistanceDistanceleftright, text
Main.moveStage2 = function(distance, totalDistance){if(this.background.regX > Main.width){this.bear.x += distance;
		this.score.x += distance;
		this.background.regX -= distance;
		
		Main.moveGuide(distance, totalDistance);}else{Main.moveStage = Main.moveStage3;
...
// 3, text
Main.moveStage3 = function(distance, totalDistance){this.background.regX -= distance;
	if(this.background.regX <= 0){this.background.regX += Main.width;}
	Main.moveGuide(distance, totalDistance);}

// text
Main.guideMoveLength = 0;
Main.guideMoveIndex = 1;
Main.moveGuide = function(distance, totalDistance){this.guide.x += distance;
	// text
	this.guideMoveLength += distance;
	if(this.guideMoveLength >= this.guideDistance + this.guideWidth){this.reset.guide(++Main.guideMoveIndex + "00M");
		this.guideMoveLength = 0;}}

Main.getTotalDistance = function(distance){Main.log(distance);
...
Main.calculateAngle = function(x){Main.log("text" + x);
	var y = Main.minAngle + Main.maxAngle * Math.sin(Math.PI * x / 200);
	y += Math.round(Math.random() * Main.randomAngle) * (Math.random() < 0.5? -1: 1);
	return Math.ceil(y);}

Main.gameOverScoreUtil = {dropDown: function(score, x){Main.reset.gameOverScore(score);
		Main.gameOverScore.x = x;
		createjs.Tween.get(Main.gameOverScore, {loop: false}).to({y: Main.floorLine - 60}, 800, createjs.Ease.quintIn);}};

// Game Over
Main.on("gameOver", function(event, distance, x){Main.log("Game Over:" + Main.getTotalDistance(distance));
	var score = Main.getTotalDistance(distance);
	var hasNewMaxScore = score > Main.maxScore;
	Main.maxScore = Math.max(Main.maxScore, score);
	try{localStorage["penguinMaxScore"] = Main.maxScore;}catch(e){}
	if(distance <= 0){Main.reset.endPenguin(true);
		Main.bear.gotoAndPlay("shootNull");
		//Main.gameOverScoreUtil.dropDown("0", Main.penguin.x + 50);}else{//Main.gameOverScoreUtil.dropDown(score, x);}
	setTimeout(function(){Main.endGame(score);
		if(hasNewMaxScore && score > 700){btGame.playScoreMsg("textlarge text, large text!text, textscoretextdowntext");
			btGame.setShare({title: "text"text"middletext"+Main.maxScore+"text, textdowntext!"});}}, 1200);});
